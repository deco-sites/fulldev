...
export default function Progress({nivel}: Props){
    ...
    const [devNivel, setDevNivel] = useState(nivel);
    ...
    return (
        <>
            <p class={`text-base font-semibold ${getTextColorNivel(devNivel)} font-sans mb-2`}>Level: {nivel}</p>
            <div class="w-48 bg-slate-300 h-5 border border-slate-300 rounded">
                <div class={`${getLengthNivel(devNivel)} h-full ${getColorNivel(devNivel)} rounded-e`}></div>
            </div>
        </>
    )
}